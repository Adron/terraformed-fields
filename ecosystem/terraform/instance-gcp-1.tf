# Elasticsearch One
resource "google_compute_instance" "gcp-brood-server-1" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "./setup-app.sh ${google_compute_instance.gcp-brood-server-1.network_interface.0.access_config.0.assigned_nat_ip}"]
